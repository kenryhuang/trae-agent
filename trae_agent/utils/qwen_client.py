...
"""Qwen API client wrapper with tool integration."""
...
class QwenClient(BaseLLMClient):
    """Qwen client wrapper with tool schema generation."""
    ...
    def __init__(self, model_parameters: ModelParameters):
        ...
        if self.api_key == "":
            raise ValueError(
                "Qwen API key not provided. Set QWEN_API_KEY in environment variables or config file."
            )

        if self.base_url is None or self.base_url == "":
            self.base_url: str | None = os.getenv("QWEN_API_BASE_URL")

        if self.base_url is None:
            raise ValueError(
                "Qwen API base url not provided. Set QWEN_API_BASE_URL in environment variables or config file."
            )
    ...
    def chat(
        ...
        """Send chat messages to Qwen with optional tool support."""
        qwen_messages = self.parse_messages(messages)
        if reuse_history:
            self.message_history = self.message_history + qwen_messages
        else:
            self.message_history = qwen_messages
        ...
        if response is None:
            raise ValueError(
                f"Failed to get response from Qwen after max retries: {error_message}"
            )
    ...
    def supports_tool_calling(self, model_parameters: ModelParameters) -> bool:
        ...
        tool_capable_models = ["qwen-max", "qwen-max-1201", "qwen-max-longcontext"]
        return any(model in model_parameters.model for model in tool_capable_models)

    def parse_messages(
        self, messages: list[LLMMessage]
    ) -> list[ChatCompletionMessageParam]:
        """Parse messages to Qwen format."""
        qwen_messages: list[ChatCompletionMessageParam] = []
        for msg in messages:
            if msg.tool_call:
